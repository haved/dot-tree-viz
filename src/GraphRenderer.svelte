<script lang="ts">
  ...
  function updateSvgHighlights() {
    ...
    for (const [elementId, color] of highlightedElements.entries()) {
      ...
      if (elementInSvg !== null) cloneAllStrokesAsHighlight(elementInSvg, fullId, color);
    }
  }

  function cloneAllStrokesAsHighlight(node: Element, fullId: string, color: string) {
    if (node.hasAttribute('id') && node.id !== fullId) return;

    for (const child of node.children) {
      if (child.classList.contains('__highlight__')) continue;
  ...
  function onWheel(event) {
    ...
    const posInSvgX = (mouseX - panX) / oldScale;
    const posInSvgY = (mouseY - panY) / oldScale;
    panX = mouseX - posInSvgX * scale;
    panY = mouseY - posInSvgY * scale;
  }
</script>
...
<style>
  ...
  .svgContainer {
    ...
    & > svg {
      ...
      left: 0;
      user-select: none;
      transform: translateX(var(--panX)) translateY(var(--panY)) scale(var(--scale));
